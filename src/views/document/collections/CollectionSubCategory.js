...
import { getCollectionSubCategory,oneDocumentCategory } from 'services/api';
...
const CollectionSubCategory = () => {
   ...
   const [mainCategoryName, setMainCategoryName] = useState('');
    ...
    async function fetchData() {
              ...
              await getCollectionSubCategory(params.id)
            .then((res) => {
        ...
        <Permission name="documentsView">
            <Grid container spacing={2}>
              <Grid item xs={12} md={12}>
                        <Link to={`../collection/category/main/edit/${params.id}`}>
                            <Box
                                sx={{
                ...
                <Permission name="documentsAdd" type="button">
                    <Grid item xs={12} md={12}>
                        <Box sx={{ textAlign: 'right', marginTop: '20px' }}>
                            <Link to={`../collection/category/sub/add/${params.id}`}>
              ...
              {categories.map((category, index) => (
                    <Grid item xs={12} md={4} key={index}>
                        <Link to={`../collection/category/sub/one/${category._id}`}>
                            <SubCard>
